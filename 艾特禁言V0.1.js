...
export class example extends plugin {
    ...
    isExempt(e, targetQQ) {
        return config.masterQQ.includes(String(e.sender.user_id)) || // 主人QQ号豁免
               ...
               (!e.group.is_admin && !e.group.is_owner); // 非管理员和群主豁免
    }

    // 更新用户的艾特历史
    updateAtHistory(userId, timeWindow) {
        const now = Date.now();
        const history = atHistory[userId] || [];
        atHistory[userId] = history.filter(timestamp => now - timestamp < timeWindow * 1000); // 过滤掉超出时间窗口的记录
