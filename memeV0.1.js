...
const baseUrl = 'http://www.a1panda.com:2233'
/**
 * 机器人发表情是否引用回复用户
 * @type {boolean}
 */
  ...
  async init() {
    ...
    if (fs.existsSync('data/memes/infos.json')) {
      ...
      for (const key in infos) {
        if (infos[key].params) {
          infos[key].params_type = infos[key].params
          delete infos[key].params
        }
      }
    }
   ...
   */
    ...
    if (info.params_type.max_images > 0) {
      ...
      if (imgUrls.length < info.params_type.min_images) {
        // 如果数量不够，补上发送者头像
        while (imgUrls.length < info.params_type.min_images) {
          imgUrls.push(await getAvatar(e))
        }
      }
    ...
    if (!text && info.params_type.min_texts > 0) {
      if (e.message.filter(m => m.type === 'at').length > 0) {
        ...
        if (!text) text = (e.sender.card || e.sender.nickname || '默认文本').trim()
      } else {
        text = (e.sender.card || e.sender.nickname || '默认文本').trim()
      }
    }
    let texts = (text || '').trim().split('/', info.params_type.max_texts)
    ...
    if (info.params_type.max_texts > 0 && formData.getAll('texts').length === 0) {
      if (formData.getAll('texts').length < info.params_type.max_texts) {
        if (e.message.filter(m => m.type === 'at').length > 0) {
          let atText = _.trim(e.message.filter(m => m.type === 'at')[0].text, '@')
          formData.append('texts', atText || (e.sender.card || e.sender.nickname || '默认文本').trim())
        } else {
          formData.append('texts', (e.sender.card || e.sender.nickname || '默认文本').trim())
        }
      }
    }
    if (e.message.filter(m => m.type === 'at').length > 0) {
      ...
      userInfos.forEach(ui => {
        ...
        if (user) {
          ui.gender = user.sex
          ui.text = user.card || user.nickname || String(ui.qq)
        }
      })
    }
    if (!userInfos) {
...
const detail = code => {
  let d = infos[code];
  if (!d?.params_type) return '暂无该表情包信息';
